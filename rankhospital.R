...
{
  ## Reads outcome data
  hosp_data <- read.csv("outcome-of-care-measures.csv", sep = ",")
  
  ## Checks that state and outcome are valid
  valid.states <- c("AL", "AK", "AZ", "AR", "CA", "CO", "CT", "DE", "FL", "GA", "HI", "ID", "IL", "IN", "IA", "KS", "KY", "LA", "ME", "MD", "MA", "MI", "MN", "MS", "MO", "MT", "NE", "NV", "NH", "NJ", "NM", "NY", "NC", "ND", "OH", "OK", "OR", "PA", "RI", "SC", "SD", "TN", "TX", "UT", "VT", "VA", "WA", "WV", "WI", "WY")
  valid.outcomes <- c("heart attack", "heart failure", "pneumonia")
  if (!is.element(state, valid.states)) stop("invalid state")
  if (!is.element(outcome, valid.outcomes)) stop("invalid outcome")
  
  ## Returns hospital name in that state with lowest 30-day death
  data <- hosp_data[hosp_data$State == state,]
  header_name <- NULL
  ...
  else if (outcome == "pneumonia") header_name <- "Hospital.30.Day.Death..Mortality..Rates.from.Pneumonia"
